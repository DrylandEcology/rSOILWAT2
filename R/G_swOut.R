...
KeyComments <- c("","/* max., min, average temperature (C) */", "/* total precip = sum(rain, snow), rain, snow-fall, snowmelt, and snowloss (cm)     */","/* water to infiltrate in top soil layer (cm), runoff (cm); (not-intercepted rain)+(snowmelt-runoff) */",
...
swOUT_key <- setClass(Class="swOUT_key",representation(mykey="integer",myobj="integer",period="integer",sumtype="integer",use="logical",first="integer",last="integer",first_orig="integer",last_orig="integer",outfile="character"),
		prototype=prototype(mykey=as.integer(c(0:4,0,6:18,0,20:25,0,27)),myobj=as.integer(c(0,rep(2,4),0,rep(4,13),0,rep(4,6),0,5)),period=as.integer(c(4,1,2,3,1,4,2,3,0,1,2,3,0,3,0,1,2,0,1,4,3,0,0,1,2,2,4,3)),sumtype=as.integer(c(0,2,1,1,1,0,rep(2,7),rep(1,6),0,1,1,1,2,1,2,0,0)),use=c(FALSE,rep(TRUE,4),FALSE,rep(TRUE,13),FALSE,rep(TRUE,6),FALSE,TRUE),
...
swOUT <- setClass(Class="swOUT",representation(outputSeparator="character",timePeriods="integer",useTimeStep="logical"),prototype=prototype(outputSeparator="\t",timePeriods=as.integer(c(3,2,1,0)), useTimeStep=TRUE),contains="swOUT_key")
setMethod(f="swClear",
		signature="swOUT",
		definition=function(object) {
