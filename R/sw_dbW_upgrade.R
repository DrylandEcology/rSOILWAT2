...
strip_weather_data_NA_columns <- function(weather_data) {

    # Create list/vector to store column names for new class
    new_col_names <- list()

    # Loop through all columns of the newest version of `weatherData`
    for (column in colnames(weather_data@data)) {
        filled_with_more_than_NA <- !is.na(weather_data@data[, column])

        # Check if there is any data provided in the column (not all NA)
        if(any(filled_with_more_than_NA)) {
            # Add column name to vector for the new class
            new_col_names <- append(new_col_names, column)
        }
    }
    num_new_columns <- length(new_col_names)
    new_col_names <- paste(new_col_names)

    # Construct new class with dynamic dimension names/sizes
    setClass(
    "newWeatherClass",
    slots = c(data = "matrix", year = "integer"),
    prototype = list(
        data = array(
            data = c(1:366, rep(NA, 366 * num_new_columns)),
            dim = c(366, num_new_columns),
    ...
    # Transfer data from old to new version
    return_object <- new("newWeatherClass")
    return_object@data <- weather_data@data[, new_col_names]
