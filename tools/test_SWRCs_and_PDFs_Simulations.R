...
for (k0 in seq_along(fadj_ppts)) {
  ...
  swout <- lapply(
    ...
    function(sp) {
      ...
      rSOILWAT2::swWeather_MonScalingParams(sw_in)[, "PPT"] <- fadj_ppts[k0]
  ...
  for (k1 in seq_along(list_plot_vars)) {
    ...
    fname_fig <- file.path(
      ".",
      paste0(
        "Fig_SOILWAT2_SWRCs-PDFs_",
        list_plot_vars[[k1]][["slot"]],
        "_PPT", round(100 * fadj_ppts[k0]), "pct",
        ".pdf"
      )
    )
