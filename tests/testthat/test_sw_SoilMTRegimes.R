...
STR_tests <- list(Ex1 = "Cryic", Ex2 = NULL, Ex3 = "Cryic", Ex4 = "Cryic")
...
test_that("SMTR", {
  for (it in tests) {
    ...
    if (!swWeather_UseMarkov(sw_in)) {
      sw_weather <- readRDS(file.path(dir_test_data,
        paste0(it, "_weather.rds")))
      ...
      expect_true(all(colnames(SMTR[["SMR"]]) %in%
          c(SMR_names(), SMRq_names())))
