...
```{r, help, results = "hide"}
  ...
  # Code of a specific function, e.g.,
  sw_exec
```
...
## Convert a `rSOILWAT2` weather object
```{r, daymetr_data2}
  if (!inherits(dm_Laramie, "try-error")) {
    # Task: convert `DayMet` data `dm_Laramie` into a data.frame with the
    # columns `Year`, `DOY`, `Tmax_C`, `Tmin_C`, `PPT_cm` and
    # create a `rSOILWAT2` weather object list with
    # function `dbW_dataframe_to_weatherData`
...
```{r, daymetr_data3}
  if (!inherits(dm_Laramie, "try-error")) {
    # Task: Add leap days to data.frame and estimate those with
