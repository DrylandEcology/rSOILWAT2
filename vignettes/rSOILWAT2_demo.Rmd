...
```{r, weather_daymetr, results = "hide", message = FALSE, warning = FALSE}
  ...
  if (
    ...
    if (!inherits(mm_dm, "try-error")) {
      ...
      # Correct weather values, e.g., min > max, percent > 100%
      wdata <- rSOILWAT2::dbW_fixWeather(
      ...
      sw_in@weather@dailyInputFlags <- rSOILWAT2::calc_dailyInputFlags(wdata)
...
```{r, soils_sda, results = "hide", message = FALSE, warning = FALSE}
  if (
    ...
    if (inherits(soils_sda, "try-error")) {
      warning(
        "Vignette was not able to extract soils from NRCS SDA!", call. = FALSE
      )
...
### Soil organic matter
```{r, som, results = "hide"}
  # Set soil organic matter to zero if we don't have values
  if (anyNA(soil_new[, "som_frac"])) {
    soil_new[, "som_frac"] <- 0
  }
```
...
### Potential bare-soil evaporation coefficients estimated from soil texture
```{r, evco, results = "hide"}
  # Option 1: Estimate coefficients and provide as inputs
  ...
  # Option 2: Do not provide inputs and estimate with (identical) internal code
  rSOILWAT2::swSite_PotSoilEvCoMethod(sw_in) <- 1L
  soil_new[, "EvapBareSoil_frac"] <- NA
```
...
```{r, vegcomp}
  ...
  # Assign fractional cover values to rSOILWAT2 input object (using v1 types)
  ids <- rSOILWAT2::mapVegTypes("2from1", order = "SOILWAT2")
  rSOILWAT2::swProd_Composition(sw_in) <- 0
  rSOILWAT2::swProd_Composition(sw_in)[c(which(ids > 0L), 7L)] <-
    veg_cover[["Rel_Abundance_L1"]][c(ids, 5L)]

  # Assign fractional cover values to rSOILWAT2 input object (using v2 types)
  rSOILWAT2::swProd_Composition(sw_in) <- 0
  rSOILWAT2::swProd_Composition(sw_in) <- veg_cover[["Rel_Abundance_L2"]]
```
...
```{r, veg}
  ...
  rSOILWAT2::swProd_MonProd_veg(sw_in, "grassC3")[, v2] <-
    veg_biom[["grass"]][, v1]
  rSOILWAT2::swProd_MonProd_veg(sw_in, "shrub")[, v2] <-
    veg_biom[["shrub"]][, v1]
```
...
```{r, roots}
  ...
  trco_type_by_veg <- list(
    treeNL = if (veg_cover[["Rel_Abundance_L0"]][["Trees"]] > 0) {
      "Bradfordetal2014_LodgepolePine"
    } else {
      "FILL"
    },
    treeBL = if (veg_cover[["Rel_Abundance_L0"]][["Trees"]] > 0) {
      "Bradfordetal2014_LodgepolePine"
    } else {
      "FILL"
    },
    ...
    forbs = if (veg_cover[["Rel_Abundance_L0"]][["Forbs"]] > 0) {
      "SchenkJackson2003_PCdry_forbs"
    } else {
      "FILL"
    },
    grassC3 = if (veg_cover[["Rel_Abundance_L0"]][["Grasses_C3"]] > 0) {
      "SchenkJackson2003_PCdry_grasses"
    } else {
      "FILL"
    },
    grassC4 = if (veg_cover[["Rel_Abundance_L0"]][["Grasses_C4"]] > 0) {
      "SchenkJackson2003_PCdry_grasses"
    } else {
      "FILL"
    },
  ...
  vn <- rSOILWAT2::namesVegTypes("v2")
  v2 <- paste0("TrCo_", vn)
  soil_new[, v2] <- veg_roots[, vn, drop = FALSE]
```
...
```{r, soils_swrc}
  ...
  # Provide `SWRC` parameters for organic matter
  rSOILWAT2::swSoils_omSWRCp(sw_in) <-
    rSOILWAT2::sw2_list_omSWRCp[[tmp[["swrc_name"]]]]

  # Alternatively (and equivalently),
  # set soil properties and SWRC parameters with one assignment
  rSOILWAT2::set_swSoils(sw_in) <- list(
    Layers = soil_new,
    SWRCp = swrcp,
    omSWRCp = rSOILWAT2::sw2_list_omSWRCp[[tmp[["swrc_name"]]]]
  )
```
