---
title: >-
          How rSOILWAT2 calculates NCRS soil moisture regimes and soil
          temperature regimes
author: | 
...
```{r, install, results = "hide"}
  ...
  if (inherits(v, "try-error") || v < "2.4.0") {
    # Install rSOILWAT2 from github repository
    args <- paste("clone -b feature_fun_from_rSFSW2 --single-branch --recursive",
      "https://github.com/DrylandEcology/rSOILWAT2.git rSOILWAT2")
    system2(command = "git", args = args)
    tools::Rcmd(args = paste("INSTALL rSOILWAT2 --preclean --clean"))
  }
...
```{r, SMTRs, collapse = TRUE}
  ...
  stopifnot(as.logical(SMTR[c("regimes_done", "has_simulated_SoilTemp",
    "has_realistic_SoilTemp")]), SMTR[["SMR_normalyears_N"]] > 2)
...
```{r, rr, collapse = TRUE}
  ...
  RR <- calc_RRs_Chambers2014(Tregime, Sregime, MAP_mm = 10 * clim[["MAP_cm"]])
  print(RR)

  # Determine resilience & resistance classes (sensu Maestas et al. 2016):
  RR <- calc_RRs_Maestas2016(Tregime, Sregime)
  print(RR)
```
