...
void SW_SWC_new_year(void) {
	...
	{
		if (reset) {
			SW_Soilwat.swc[Today][lyr] = SW_Soilwat.swc[Yesterday][lyr] = SW_Site.lyr[lyr]->swc_init;
...
RealD SW_SWC_vol2bars(RealD lyrvolcm, LyrIndex n) {
	...
	if (missing(lyrvolcm) || ZRO(lyrvolcm))
		return 0.0;

	if (GT(lyrvolcm, 0.0)) {
		theta1 = (lyrvolcm / lyr->width) * 100.;
		swp = lyr->psis / pow2(theta1 / lyr->thetas, lyr->b) / BARCONV;
	} else {
...
RealD SW_SWC_bars2vol(RealD bars, LyrIndex n) {
	...
	bars *= BARCONV;
	p = pow2(lyr->psis / bars, lyr->binverse);
	t = lyr->thetas * p * 0.01;
	return (t);
...
RealD SW_SWC_SWCres(RealD sand, RealD clay, RealD porosity) {
	...
	res = -0.0182482 + 0.00087269 * sand + 0.00513488 * clay + 0.02939286 * porosity - 0.00015395 * pow2(clay, 2) - 0.0010827 * sand * porosity
			- 0.00018233 * pow2(clay, 2) * pow2(porosity, 2) + 0.00030703 * pow2(clay, 2) * porosity - 0.0023584 * pow2(porosity, 2) * clay;
