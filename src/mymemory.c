...
/*-------------------------------------------
 ...
 -------------------------------------------*/
  ...
  if (p == NULL)
    LogError(stderr, LOGFATAL, "Out of memory in %s()",
             funcname);
 ...
 * defined by pbNew:sizeNew.  The function returns swTRUE if it
 * successfully creates the log information; swFALSE otherwise.
 *
 ...
 * never return swFALSE.
 *
 * The reason fValidPointer returns a flag at all (always swTRUE)
