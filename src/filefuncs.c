...
FILE * OpenFile(const char *name, const char *mode) {
	...
	fp = fopen(name, mode);
	if (isnull(fp))
		LogError(stdout, LOGERROR | LOGEXIT, "Cannot open file %s:", name);
	return (fp);
}
...
void CloseFile(FILE **f) {
	...
	if (*f == NULL ) {
		if (logWarn)
			Rprintf("WARNING: Tried to close file that exist/isn't open!!!\n");
		return;
	}
	fclose(*f);
...
Bool FileExists(const char *name) {
	...
	if (0 == stat(name, &statbuf))
		result = (statbuf.st_mode & S_IFREG) ? swTRUE : swFALSE;
...
Bool DirExists(const char *dname) {
	...
	if (0 == stat(dname, &statbuf))
		result = (statbuf.st_mode & S_IFDIR) ? swTRUE : swFALSE;
...
Bool MkDir(const char *dname) {
	 ...
	 * if MkDir returns swFALSE, check errno.
	 *
	 * Notes:
	...
	*c; /* duplicate of dname so we don't change it */
	...
	if (NULL == (c = strdup(dname))) {
		if (logFatl) {
			Rprintf("Out of memory making string in MkDir()");
			Rprintf("EXIT -1");
			error("@ filefuncs.c NULL == (c = strdup(dname))");
		}
		//exit(-1);
	}
...
Bool RemoveFiles(const char *fspec) {
	 ...
	 *   or /here/now/files
	 * Returns swTRUE if all files removed, swFALSE otherwise.
	 * Check errno if return is swFALSE.
	 */
