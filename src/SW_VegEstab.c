...
void onSet_SW_VES(SEXP VES) {
	...
	if (LOGICAL(use)[0] == swFALSE) {
		//LogError(logfp, LOGNOTE, "Establishment not used.\n");
		SW_VegEstab.use = swFALSE;
	} else {
		nSPPS = LENGTH(GET_SLOT(VES,install("fileName")));
		if (nSPPS == 0) {
...
void onGet_SW_VES_spps(SEXP SPP) {
	int i;
	SW_VEGESTAB_INFO *v;
	SEXP name, estab_lyrs, barsGERM, barsESTAB, min_pregerm_days, max_pregerm_days, min_wetdays_for_germ, max_drydays_postgerm, min_wetdays_for_estab, min_days_germ2estab,
	...
	SET_SLOT(SPP, install("fileName"), name);
...
void onSet_SW_VES_spp(SEXP SPP, IntU i) {
	...
	/* check for valid name first */
	if (strlen(CHAR(GET_SLOT(SPP, install("fileName")))) > MAX_SPECIESNAMELEN) {
		LogError(logfp, LOGFATAL, "Species name too long (> 4 chars).\n\tTry again.\n");
	} else {
		strcpy(v->sppname, CHAR(GET_SLOT(SPP, install("fileName"))));
	}
}
...
static void _sanity_check(unsigned int sppnum) {
	...
	if (v->min_swc_germ < lyr[0]->swc_wiltpt) {
		LogError(logfp, LOGFATAL, "%s : Minimum swc for germination (%.4f) < wiltpoint (%.4f)", MyFileName, v->min_swc_germ, lyr[0]->swc_wiltpt);
	}

	if (v->min_swc_estab < lyr[0]->swc_wiltpt) {
		LogError(logfp, LOGFATAL, "%s : Minimum swc for establishment (%.4f) < wiltpoint (%.4f)", MyFileName, v->min_swc_estab, lyr[0]->swc_wiltpt);
	}
