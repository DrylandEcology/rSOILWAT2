 ...
 */
IntU lyrTrRegions_Tree[MAX_LAYERS], lyrTrRegions_Shrub[MAX_LAYERS], lyrTrRegions_Grass[MAX_LAYERS];
RealD lyrSWC[MAX_LAYERS], lyrDrain[MAX_LAYERS], lyrTransp_Tree[MAX_LAYERS], lyrTransp_Shrub[MAX_LAYERS], lyrTransp_Grass[MAX_LAYERS], lyrTranspCo_Tree[MAX_LAYERS],
		lyrTranspCo_Shrub[MAX_LAYERS], lyrTranspCo_Grass[MAX_LAYERS], lyrEvap_Tree[MAX_LAYERS], lyrEvap_Shrub[MAX_LAYERS], lyrEvap_Grass[MAX_LAYERS], lyrEvapCo[MAX_LAYERS],
		lyrFieldCaps[MAX_LAYERS], lyrWidths[MAX_LAYERS], lyrWiltpts[MAX_LAYERS], lyrHalfWiltpts[MAX_LAYERS], lyrSWCMins[MAX_LAYERS], lyrSWCatSWPcrit_Tree[MAX_LAYERS],
		lyrSWCatSWPcrit_Shrub[MAX_LAYERS], lyrSWCatSWPcrit_Grass[MAX_LAYERS], lyrPsis[MAX_LAYERS], lyrThetas[MAX_LAYERS], lyrBetas[MAX_LAYERS], lyrBetaInv[MAX_LAYERS],
		lyrSumTrCo[MAX_TRANSP_REGIONS + 1], lyrHydRed_Tree[MAX_LAYERS], lyrHydRed_Shrub[MAX_LAYERS], lyrHydRed_Grass[MAX_LAYERS], lyrImpermeability[MAX_LAYERS],
		lyrSWCsaturated[MAX_LAYERS], lyroldsTemp[MAX_LAYERS], lyrsTemp[MAX_LAYERS], lyrbDensity[MAX_LAYERS];
...
/* --------------------------------------------------- */
 void SW_Water_Flow(void) {
	...
	RealD swpot_avg_tree, swpot_avg_shrub, swpot_avg_grass, soil_evap_tree, soil_evap_shrub, soil_evap_grass, soil_evap_rate_tree = 1., soil_evap_rate_shrub = 1.,
			soil_evap_rate_grass = 1., transp_tree, transp_shrub, transp_grass, transp_rate_tree = 1., transp_rate_shrub = 1., transp_rate_grass = 1., snow_evap_rate,
			surface_evap_tree_rate, surface_evap_shrub_rate, surface_evap_grass_rate, surface_evap_litter_rate, surface_evap_standingWater_rate, grass_h2o, shrub_h2o,
			tree_h2o, litter_h2o, litter_h2o_help, surface_h2o, h2o_for_soil = 0., ppt_toUse, snowmelt, snowdepth_scale_grass = 1., snowdepth_scale_shrub = 1.,
			snowdepth_scale_tree = 1., rate_help;
...
//	month = SW_Model.month; /* base0 */
	...
	infiltrate_water_high(lyrSWC, lyrDrain, &drainout, h2o_for_soil, SW_Site.n_layers, lyrFieldCaps, lyrSWCsaturated, lyrImpermeability, &standingWater[Today]);
	...
	SW_Soilwat.pet = SW_Site.pet_scale
			* petfunc(doy, SW_Weather.now.temp_avg[Today], SW_Site.latitude, SW_Site.altitude, SW_Site.slope, SW_Site.aspect,
					...
					SW_Sky.r_humidity_daily[doy], SW_Sky.windspeed_daily[doy], SW_Sky.cloudcov_daily[doy], SW_Sky.transmission_daily[doy]);
	...
	if (GT(SW_VegProd.fractionTree, 0.) && GT(snowdepth_scale_tree, 0.)) { /* trees present AND trees not fully covered in snow */
		...
		if (EQ(SW_Soilwat.snowpack[Today], 0.)) { /* bare-soil evaporation only when no snow */
			pot_soil_evap(&soil_evap_rate_tree, SW_Site.n_evap_lyrs, lyrEvapCo, SW_VegProd.tree.total_agb_daily[doy], soil_evap_tree, SW_Soilwat.pet, SW_Site.evap.xinflec,
					SW_Site.evap.slope, SW_Site.evap.yinflec, SW_Site.evap.range, lyrWidths, lyrSWC, SW_VegProd.tree.Es_param_limit);
			soil_evap_rate_tree *= SW_VegProd.fractionTree;
		} else {
			soil_evap_rate_tree = 0.;
		}

		transp_weighted_avg(&swpot_avg_tree, SW_Site.n_transp_rgn, SW_Site.n_transp_lyrs_tree, lyrTrRegions_Tree, lyrTranspCo_Tree, lyrSWC);
	...
	if (GT(SW_VegProd.fractionShrub, 0.) && GT(snowdepth_scale_shrub, 0.)) { /* shrubs present AND shrubs not fully covered in snow */
		...
		if (EQ(SW_Soilwat.snowpack[Today], 0.)) { /* bare-soil evaporation only when no snow */
			pot_soil_evap(&soil_evap_rate_shrub, SW_Site.n_evap_lyrs, lyrEvapCo, SW_VegProd.shrub.total_agb_daily[doy], soil_evap_shrub, SW_Soilwat.pet, SW_Site.evap.xinflec,
					SW_Site.evap.slope, SW_Site.evap.yinflec, SW_Site.evap.range, lyrWidths, lyrSWC, SW_VegProd.shrub.Es_param_limit);
			soil_evap_rate_shrub *= SW_VegProd.fractionShrub;
		} else {
			soil_evap_rate_shrub = 0.;
		}

		transp_weighted_avg(&swpot_avg_shrub, SW_Site.n_transp_rgn, SW_Site.n_transp_lyrs_shrub, lyrTrRegions_Shrub, lyrTranspCo_Shrub, lyrSWC);
	...
	if (GT(SW_VegProd.fractionGrass, 0.) && GT(snowdepth_scale_grass, 0.)) { /* grasses present AND grasses not fully covered in snow */
		...
		if (EQ(SW_Soilwat.snowpack[Today], 0.)) { /* bare-soil evaporation only when no snow */
			pot_soil_evap(&soil_evap_rate_grass, SW_Site.n_evap_lyrs, lyrEvapCo, SW_VegProd.grass.total_agb_daily[doy], soil_evap_grass, SW_Soilwat.pet, SW_Site.evap.xinflec,
					SW_Site.evap.slope, SW_Site.evap.yinflec, SW_Site.evap.range, lyrWidths, lyrSWC, SW_VegProd.grass.Es_param_limit);
			soil_evap_rate_grass *= SW_VegProd.fractionGrass;
		} else {
			soil_evap_rate_grass = 0.;
		}

		transp_weighted_avg(&swpot_avg_grass, SW_Site.n_transp_rgn, SW_Site.n_transp_lyrs_grass, lyrTrRegions_Grass, lyrTranspCo_Grass, lyrSWC);
	...
	rate_help = surface_evap_tree_rate + surface_evap_shrub_rate + surface_evap_grass_rate + surface_evap_litter_rate + surface_evap_standingWater_rate + soil_evap_rate_tree
			+ transp_rate_tree + soil_evap_rate_shrub + transp_rate_shrub + soil_evap_rate_grass + transp_rate_grass;
	...
	SW_Soilwat.aet += snow_evap_rate; //Why do we add here if we just set it to zero?
	...
	if (GT(SW_VegProd.fractionTree, 0.) && GT(snowdepth_scale_tree, 0.)) {
		...
		remove_from_soil(lyrSWC, lyrEvap_Tree, &SW_Soilwat.aet, SW_Site.n_evap_lyrs, lyrEvapCo, soil_evap_rate_tree, lyrHalfWiltpts);

		/* remove transp from swc */
		remove_from_soil(lyrSWC, lyrTransp_Tree, &SW_Soilwat.aet, SW_Site.n_transp_lyrs_tree, lyrTranspCo_Tree, transp_rate_tree, lyrSWCatSWPcrit_Tree);
	} else {
	...
	if (GT(SW_VegProd.fractionShrub, 0.) && GT(snowdepth_scale_shrub, 0.)) {
		/* remove bare-soil evap from swc */
		remove_from_soil(lyrSWC, lyrEvap_Shrub, &SW_Soilwat.aet, SW_Site.n_evap_lyrs, lyrEvapCo, soil_evap_rate_shrub, lyrHalfWiltpts);

		/* remove transp from swc */
		remove_from_soil(lyrSWC, lyrTransp_Shrub, &SW_Soilwat.aet, SW_Site.n_transp_lyrs_shrub, lyrTranspCo_Shrub, transp_rate_shrub, lyrSWCatSWPcrit_Shrub);
	} else {
		/* Set daily array to zero, no evaporation or transpiration */
		LyrIndex i;
		for (i = 0; i < SW_Site.n_evap_lyrs;)
	...
	if (GT(SW_VegProd.fractionGrass, 0.) && GT(snowdepth_scale_grass, 0.)) {
		/* remove bare-soil evap from swc */
		remove_from_soil(lyrSWC, lyrEvap_Grass, &SW_Soilwat.aet, SW_Site.n_evap_lyrs, lyrEvapCo, soil_evap_rate_grass, lyrHalfWiltpts);

		/* remove transp from swc */
		remove_from_soil(lyrSWC, lyrTransp_Grass, &SW_Soilwat.aet, SW_Site.n_transp_lyrs_grass, lyrTranspCo_Grass, transp_rate_grass, lyrSWCatSWPcrit_Grass);
	} else {
		/* Set daily array to zero, no evaporation or transpiration */
		LyrIndex i;
		for (i = 0; i < SW_Site.n_evap_lyrs;)
	...
	if (SW_VegProd.grass.flagHydraulicRedistribution && GT(SW_VegProd.fractionGrass, 0.) && GT(SW_VegProd.grass.biolive_daily[doy], 0.)) {
		hydraulic_redistribution(lyrSWC, lyrWiltpts, lyrTranspCo_Grass, lyrHydRed_Grass, SW_Site.n_layers, SW_VegProd.grass.maxCondroot, SW_VegProd.grass.swp50,
				SW_VegProd.grass.shapeCond, SW_VegProd.fractionGrass);
	}
	if (SW_VegProd.shrub.flagHydraulicRedistribution && GT(SW_VegProd.fractionShrub, 0.) && GT(SW_VegProd.shrub.biolive_daily[doy], 0.)) {
		hydraulic_redistribution(lyrSWC, lyrWiltpts, lyrTranspCo_Shrub, lyrHydRed_Shrub, SW_Site.n_layers, SW_VegProd.shrub.maxCondroot, SW_VegProd.shrub.swp50,
				SW_VegProd.shrub.shapeCond, SW_VegProd.fractionShrub);
	}
	if (SW_VegProd.tree.flagHydraulicRedistribution && GT(SW_VegProd.fractionTree, 0.) && GT(SW_VegProd.tree.biolive_daily[doy], 0.)) {
		hydraulic_redistribution(lyrSWC, lyrWiltpts, lyrTranspCo_Tree, lyrHydRed_Tree, SW_Site.n_layers, SW_VegProd.tree.maxCondroot, SW_VegProd.tree.swp50,
	...
	infiltrate_water_low(lyrSWC, lyrDrain, &drainout, SW_Site.n_layers, SW_Site.slow_drain_coeff, SLOW_DRAIN_DEPTH, lyrFieldCaps, lyrWidths, lyrSWCMins, lyrSWCsaturated,
