...
SEXP onGet_SW_VPD() {
	...
	char *cVegProd_names[] = { "Composition", "Albedo", "Cover_stcr", "CanopyHeight", "VegetationInterceptionParameters", "LitterInterceptionParameters",
			...
			"MonthlyProductionValues_grass", "MonthlyProductionValues_shrub", "MonthlyProductionValues_tree" };
	...
	//SEXP Hydraulic_maxCondroot;
	//SEXP Hydraulic_swp;
	//SEXP Hydraulic_shapeCond;

	SEXP CSWP;

	//SEXP MonthlyProductionValues, MonthlyProductionValues_names;
	//char *cMonthlyProductionValues_names[] = { "Grasslands", "Shrublands", "ForestComponent" };
	SEXP Grasslands, Grasslands_names;
	SEXP Shrublands, Shrublands_names;
	SEXP Forest, Forest_names;
	RealD *p_Grasslands, *p_Shrublands, *p_Forest;
	...
	PROTECT(Albedo = allocVector(REALSXP, 3));
	...
	PROTECT(conv_stcr = allocVector(REALSXP, 3));
	REAL(conv_stcr)[0] = v->grass.conv_stcr; //Grass
	REAL(conv_stcr)[1] = v->shrub.conv_stcr; //Shrub
	REAL(conv_stcr)[2] = v->tree.conv_stcr; //Tree
	setAttrib(conv_stcr, R_NamesSymbol, VegComp_names);

	PROTECT(Canopy = allocMatrix(REALSXP, 5, 3));
	...
	SET_VECTOR_ELT(Canopy_names, 1, VegComp_names);
	setAttrib(Canopy, R_DimNamesSymbol, Canopy_names);

	PROTECT(VegInterception = allocMatrix(REALSXP, 4, 3));
	...
	SET_VECTOR_ELT(VegInterception_names, 1, VegComp_names);
	setAttrib(VegInterception, R_DimNamesSymbol, VegInterception_names);

	PROTECT(LitterInterception = allocMatrix(REALSXP, 4, 3));
	...
	SET_VECTOR_ELT(LitterInterception_names, 1, VegComp_names);
	setAttrib(LitterInterception, R_DimNamesSymbol, LitterInterception_names);

	PROTECT(EsTpartitioning_param = allocVector(REALSXP, 3));
	...
	setAttrib(EsTpartitioning_param, R_NamesSymbol, VegComp_names);

	PROTECT(Es_param_limit = allocVector(REALSXP, 3));
	...
	setAttrib(Es_param_limit, R_NamesSymbol, VegComp_names);

	PROTECT(Shade = allocMatrix(REALSXP, 6, 3));
	p_Shade = REAL(Shade);
	...
	SET_VECTOR_ELT(Shade_names, 1, VegComp_names);
	setAttrib(Shade, R_DimNamesSymbol, Shade_names);

	PROTECT(Hydraulic_flag = allocVector(LGLSXP, 3));
	...
	setAttrib(Hydraulic_flag, R_NamesSymbol, VegComp_names);

	//PROTECT(Hydraulic_maxCondroot = allocVector(REALSXP, 3));
	//REAL(Hydraulic_maxCondroot)[0] = v->grass.maxCondroot; //Grass
	//REAL(Hydraulic_maxCondroot)[1] = v->shrub.maxCondroot; //Shrub
	//REAL(Hydraulic_maxCondroot)[2] = v->tree.maxCondroot; //Tree
	//setAttrib(Hydraulic_maxCondroot, R_NamesSymbol, VegComp_names);

	//PROTECT(Hydraulic_swp = allocVector(REALSXP, 3));
	//REAL(Hydraulic_swp)[0] = v->grass.swp50; //Grass
	//REAL(Hydraulic_swp)[1] = v->shrub.swp50; //Shrub
	//REAL(Hydraulic_swp)[2] = v->tree.swp50; //Tree
	//setAttrib(Hydraulic_swp, R_NamesSymbol, VegComp_names);

	//PROTECT(Hydraulic_shapeCond = allocVector(REALSXP, 3));
	//REAL(Hydraulic_shapeCond)[0] = v->grass.shapeCond; //Grass
	//REAL(Hydraulic_shapeCond)[1] = v->shrub.shapeCond; //Shrub
	//REAL(Hydraulic_shapeCond)[2] = v->tree.shapeCond; //Tree
	//setAttrib(Hydraulic_shapeCond, R_NamesSymbol, VegComp_names);

	PROTECT(Hydraulic = allocMatrix(REALSXP, 3, 3));
	p_Hydraulic = REAL(Hydraulic);
	p_Hydraulic[0] = v->grass.maxCondroot;
	p_Hydraulic[1] = v->grass.swp50;
	p_Hydraulic[2] = v->grass.shapeCond;
	p_Hydraulic[3] = v->shrub.maxCondroot;
	p_Hydraulic[4] = v->shrub.swp50;
	p_Hydraulic[5] = v->shrub.shapeCond;
	p_Hydraulic[6] = v->tree.maxCondroot;
	p_Hydraulic[7] = v->tree.swp50;
	p_Hydraulic[8] = v->tree.shapeCond;
	//SET_VECTOR_ELT(Hydraulic, 0, Hydraulic_flag);
	//SET_VECTOR_ELT(Hydraulic, 1, Hydraulic_maxCondroot);
	//SET_VECTOR_ELT(Hydraulic, 2, Hydraulic_swp);
	//SET_VECTOR_ELT(Hydraulic, 3, Hydraulic_shapeCond);
	...
	SET_VECTOR_ELT(Hydraulic_names,1,VegComp_names);
	setAttrib(Hydraulic, R_DimNamesSymbol, Hydraulic_names);

	PROTECT(CSWP = allocVector(REALSXP, 3));
	REAL(CSWP)[0] = v->grass.SWPcrit/-10; //Grass
	REAL(CSWP)[1] = v->shrub.SWPcrit/-10; //Shrub
	REAL(CSWP)[2] = v->tree.SWPcrit/-10; //Tree
	setAttrib(CSWP, R_NamesSymbol, VegComp_names);

	//PROTECT(MonthlyProductionValues_names = allocVector(STRSXP, 3));
	//for (i = 0; i < 3; i++)
		//SET_STRING_ELT(MonthlyProductionValues_names, i, mkChar(cMonthlyProductionValues_names[i]));
	...
	//PROTECT(MonthlyProductionValues = allocVector(VECSXP, 3));
	//SET_VECTOR_ELT(MonthlyProductionValues, 0, Grasslands);
	//SET_VECTOR_ELT(MonthlyProductionValues, 1, Shrublands);
	//SET_VECTOR_ELT(MonthlyProductionValues, 2, Forest);
	//setAttrib(MonthlyProductionValues, R_NamesSymbol, MonthlyProductionValues_names);

	//PROTECT(VegProd_names = allocVector(STRSXP, 12));
	//for (i = 0; i < 12; i++)
		//SET_STRING_ELT(VegProd_names, i, mkChar(cVegProd_names[i]));
	...
	//setAttrib(VegProd, R_NamesSymbol, VegProd_names);
...
void onSet_SW_VPD(SEXP SW_VPD) {
	...
	//PROTECT(Hydraulic_maxCondroot = VECTOR_ELT(Hydraulic, 1));
	//PROTECT(Hydraulic_swp = VECTOR_ELT(Hydraulic, 2));
	//PROTECT(Hydraulic_shapeCond = VECTOR_ELT(Hydraulic, 3));
	v->grass.maxCondroot = REAL(Hydraulic)[0]; //Grass
	v->grass.swp50 = REAL(Hydraulic)[1]; //Grass
	v->grass.shapeCond = REAL(Hydraulic)[2]; //Grass
	v->shrub.maxCondroot = REAL(Hydraulic)[3]; //Shrub
	v->shrub.swp50 = REAL(Hydraulic)[4]; //Shrub
	v->shrub.shapeCond = REAL(Hydraulic)[5]; //Shrub
	v->tree.maxCondroot = REAL(Hydraulic)[6]; //Tree
	v->tree.swp50 = REAL(Hydraulic)[7]; //Tree
