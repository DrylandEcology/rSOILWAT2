...
static void water_eqn(RealD sand, RealD clay, LyrIndex n) {
	...
	theta33 = theta33t + (1.283 * pow2(theta33t, 2) - 0.374 * theta33t - 0.015);
...
static void _read_layers(void) {
	...
	transp_ok_tree = swTRUE, /* same for transpiration coefficients */
	...
	int x;
	char *errtype = '\0';
	RealF fieldcap, wiltpt, dmin = 0.0, dmax, evco, trco_tree, trco_shrub, trco_grass, psand, pclay, bulkd, imperm, soiltemp, fval = 0;
	...
	while (GetALine(f, inbuf)) {
		...
		x = sscanf(inbuf, "%f %f %f %f %f %f %f %f %f %f %f %f", &dmax, &bulkd, &fieldcap, &wiltpt, &evco, &trco_grass, &trco_shrub, &trco_tree, &psand, &pclay, &imperm,
				&soiltemp);
		if (x < 10) {
			CloseFile(&f);
		...
		if (LE(bulkd,0.)) {
			...
			fail = swTRUE;
			fval = fieldcap;
			errtype = Str_Dup("field capacity");
		} else if (LE(wiltpt,0.)) {
			fail = swTRUE;
			fval = wiltpt;
			errtype = Str_Dup("wilting point");
		} else if (LE(psand,0.)) {
			fail = swTRUE;
			fval = psand;
...
SEXP onGet_SW_LYR() {
	...
	char *cLayers[] = { "depth_cm", "bulkDensity_g/cm^3", "fieldCapacity_cm^3/cm^3", "WiltingPoint_cm^3/cm^3", "EvapBareSoil_frac", "transpGrass_frac", "transpShrub_frac",
			"transpTree_frac", "sand_frac", "clay_frac", "impermeability_frac", "soilTemp_c" };
	RealD *p_Layers;
	RealD temp;
	...
	for (i = 0; i < (v->n_layers); i++) {
		...
		p_Layers[i + (v->n_layers) * 1] = v->lyr[i]->bulk_density;
		p_Layers[i + (v->n_layers) * 2] = v->lyr[i]->swc_fieldcap / temp;
		p_Layers[i + (v->n_layers) * 3] = v->lyr[i]->swc_wiltpt / temp;
		p_Layers[i + (v->n_layers) * 4] = v->lyr[i]->evap_coeff;
		p_Layers[i + (v->n_layers) * 5] = v->lyr[i]->transp_coeff_grass;
		p_Layers[i + (v->n_layers) * 6] = v->lyr[i]->transp_coeff_shrub;
		p_Layers[i + (v->n_layers) * 7] = v->lyr[i]->transp_coeff_tree;
		p_Layers[i + (v->n_layers) * 8] = v->lyr[i]->pct_sand;
		p_Layers[i + (v->n_layers) * 9] = v->lyr[i]->pct_clay;
...
void onSet_SW_LYR(SEXP SW_SOILS) {
	...
	for (i = 0; i < j; i++) {
		...
		dmax = p_Layers[i + j * 0];
		bulkd = p_Layers[i + j * 1];
		fieldcap = p_Layers[i + j * 2];
		wiltpt = p_Layers[i + j * 3];
		evco = p_Layers[i + j * 4];
		trco_grass = p_Layers[i + j * 5];
		trco_shrub = p_Layers[i + j * 6];
		trco_tree = p_Layers[i + j * 7];
...
static void _init_site_info(void) {
	...
	ForEachSoilLayer(s)
	{
		...
		lyr->swc_wet = GE(_SWCWetVal, 1.0) ? SW_SWC_bars2vol(_SWCWetVal, s) * lyr->width : _SWCWetVal * lyr->width;
		lyr->swc_init = GE(_SWCInitVal, 1.0) ? SW_SWC_bars2vol(_SWCInitVal, s) * lyr->width : _SWCInitVal * lyr->width;

		/* test validity of values */
		if (LT(lyr->swc_init, lyr->swc_min))
			initminflag++;
		if (LT(lyr->swc_wiltpt, lyr->swc_min))
			wiltminflag++;
		if (LE(lyr->swc_wet, lyr->swc_min)) {
			LogError(logfp, LOGFATAL, "%s : Layer %d\n"
					"  calculated swc_wet (%7.4f) <= swc_min (%7.4f).\n"
					"  Recheck parameters and try again.", MyFileName, s + 1, lyr->swc_wet, lyr->swc_min);
		}

	} /*end ForEachSoilLayer */

	if (wiltminflag) {
		LogError(logfp, LOGWARN, "%s : %d layers were found in which wiltpoint < swc_min.\n"
				"  You should reconsider wiltpoint or swc_min.\n"
				"  See site parameter file for swc_min and site.log for swc details.", MyFileName, wiltminflag);
	}

	if (initminflag) {
		LogError(logfp, LOGWARN, "%s : %d layers were found in which swc_init < swc_min.\n"
				"  You should reconsider swc_init or swc_min.\n"
				"  See site parameter file for swc_init and site.log for swc details.", MyFileName, initminflag);
	}
...
static void _echo_inputs(void) {
	...
	LogError(logfp, LOGNOTE,
			"  Lyr Width   BulkD     FieldC   WiltPt   %%Sand  %%Clay  VWC at Tree-critSWP	VWC at Shrub-critSWP	VWC at Grass-critSWP	EvCo   TrCo_Tree  TrCo_Shrub  TrCo_Grass   TrRgn_Tree   TrRgn_Shrub   TrRgn_Grass         Wet     Min      Init  Saturated    Impermeability\n");
	LogError(logfp, LOGNOTE,
			"       (cm)  (g/cm^3)  (cm/cm)  (cm/cm)   (prop) (prop)   (cm/cm)            (cm/cm)            (cm/cm)            (prop)    (prop)     (prop)    (prop)         (int) 	      	(int) 		(int) 	    (cm/cm)    (cm/cm)   (cm/cm)   (cm/cm)      (frac)\n");
	LogError(logfp, LOGNOTE,
			"  --- -----  --------   ------   ------   -----  ------   ------            ------            ------            ------    ------      ------   ------       ------   		   -----          -----   	  ----       ----      ----      ----         ----\n");
	ForEachSoilLayer(i)
	{
		LogError(logfp, LOGNOTE, "  %3d %5.1f %9.5f %8.5f %8.5f %6.2f %6.2f %6.2f %6.2f %6.2f %9.2f %9.2f %9.2f %9.2f %10d %15d %15d %15.4f %9.4f %9.4f %9.4f %9.4f\n", i + 1,
				s->lyr[i]->width, s->lyr[i]->bulk_density, s->lyr[i]->swc_fieldcap / s->lyr[i]->width, s->lyr[i]->swc_wiltpt / s->lyr[i]->width, s->lyr[i]->pct_sand,
				s->lyr[i]->pct_clay, s->lyr[i]->swc_atSWPcrit_tree / s->lyr[i]->width, s->lyr[i]->swc_atSWPcrit_shrub / s->lyr[i]->width,
				s->lyr[i]->swc_atSWPcrit_grass / s->lyr[i]->width, s->lyr[i]->evap_coeff, s->lyr[i]->transp_coeff_tree, s->lyr[i]->transp_coeff_shrub,
				s->lyr[i]->transp_coeff_grass, s->lyr[i]->my_transp_rgn_tree, s->lyr[i]->my_transp_rgn_shrub, s->lyr[i]->my_transp_rgn_grass,
				s->lyr[i]->swc_wet / s->lyr[i]->width, s->lyr[i]->swc_min / s->lyr[i]->width, s->lyr[i]->swc_init / s->lyr[i]->width,
				s->lyr[i]->swc_saturated / s->lyr[i]->width, s->lyr[i]->impermeability);

	}
	LogError(logfp, LOGNOTE, "\n  Actual per-layer values:\n");
	LogError(logfp, LOGNOTE,
			"  Lyr Width   BulkD     FieldC   WiltPt %%Sand  %%Clay	SWC at Tree-critSWP	SWC at Shrub-critSWP	SWC at Grass-critSWP	 Wet    Min      Init  Saturated	SoilTemp\n");
	LogError(logfp, LOGNOTE,
			"       (cm)  (g/cm^3)    (cm)     (cm)  (prop) (prop)   (cm)            (cm)            (cm)            (cm)   (cm)      (cm)     (cm)		(celcius)\n");
	LogError(logfp, LOGNOTE,
			"  --- -----  --------   ------   ------ ------ ------   ------            ------            ------             ----     ----     ----    ----		----\n");

	ForEachSoilLayer(i)
	{
		LogError(logfp, LOGNOTE, "  %3d %5.1f %9.5f %8.5f %8.5f %6.2f %6.2f %7.4f %7.4f %7.4f %7.4f %7.4f %8.4f %7.4f %5.4f\n", i + 1, s->lyr[i]->width,
				s->lyr[i]->bulk_density, s->lyr[i]->swc_fieldcap, s->lyr[i]->swc_wiltpt, s->lyr[i]->pct_sand, s->lyr[i]->pct_clay, s->lyr[i]->swc_atSWPcrit_tree,
				s->lyr[i]->swc_atSWPcrit_shrub, s->lyr[i]->swc_atSWPcrit_grass, s->lyr[i]->swc_wet, s->lyr[i]->swc_min, s->lyr[i]->swc_init, s->lyr[i]->swc_saturated,
				s->lyr[i]->sTemp);
	...
	LogError(logfp, LOGNOTE,
			"  Lyr         FieldCap            WiltPt              Tree-critSWP              Shrub-critSWP              Grass-critSWP              Wet            Min            Init\n");
	LogError(logfp, LOGNOTE,
	...
	ForEachSoilLayer(i)
	{
		LogError(logfp, LOGNOTE, "  %3d   %15.4f   %15.4f  %15.4f  %15.4f  %15.4f  %15.4f   %15.4f   %15.4f\n", i + 1, SW_SWC_vol2bars(s->lyr[i]->swc_fieldcap, i),
				SW_SWC_vol2bars(s->lyr[i]->swc_wiltpt, i), SW_SWC_vol2bars(s->lyr[i]->swc_atSWPcrit_tree, i), SW_SWC_vol2bars(s->lyr[i]->swc_atSWPcrit_shrub, i),
				SW_SWC_vol2bars(s->lyr[i]->swc_atSWPcrit_grass, i), SW_SWC_vol2bars(s->lyr[i]->swc_wet, i), SW_SWC_vol2bars(s->lyr[i]->swc_min, i),
