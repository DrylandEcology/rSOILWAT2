...
/* The following tests account for imprecision in the
  ...
  Update (DLM) : 6-19-2013 : These macros (iszero(), isequal(), isless2(), & ismore()) were taking a VERY long time to compute, so I wrote some new ones defined below.  I also rewrote the macros for LE() and GE() because they were making unnecessary calls.
						   ...
						   : The reason the old macros were taking sooo long is because they make lots of function calls many of which are to performance intensive functions (such as log10() and pow2()) and the macros are used EVERYWHERE in the code (I'm guessing they are being called 100's of thousands of times).
...
( (sizeof(x) == sizeof(float)) \
  ? ((x)>-( max( 10.*FLT_EPSILON, FLT_EPSILON*pow2(10., ceil(log10(max(fabs(x),FLT_EPSILON)+1.)) ) ) ) && (x)<( max( 10.*FLT_EPSILON, FLT_EPSILON*pow2(10., ceil(log10(max(fabs(x),FLT_EPSILON)+1.)) ) ) )) \
  : ((x)>-( max( 10.*DBL_EPSILON, DBL_EPSILON*pow2(10., ceil(log10(max(fabs(x),DBL_EPSILON)+1.)) ) ) ) && (x)<( max( 10.*DBL_EPSILON, DBL_EPSILON*pow2(10., ceil(log10(max(fabs(x),DBL_EPSILON)+1.)) ) ) )) )

#define isequal(x,y) \
( (sizeof(x) == sizeof(float)) \
  ? ((x)>(y)-( max( 10.*FLT_EPSILON, FLT_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), FLT_EPSILON))+1.)) ) ) ) && (x)<(y)+( max( 10.*FLT_EPSILON, FLT_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), FLT_EPSILON))+1.)) ) ) )) \
  : ((x)>(y)-( max( 10.*DBL_EPSILON, DBL_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), DBL_EPSILON))+1.)) ) ) ) && (x)<(y)+( max( 10.*DBL_EPSILON, DBL_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), DBL_EPSILON))+1.)) ) ) )) )


#define isless2(x,y) \
( (sizeof(x) == sizeof(float)) \
  ? ((x)<(y)-( max( 10.*FLT_EPSILON, FLT_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), FLT_EPSILON))+1.)) ) ) )) \
  : ((x)<(y)-( max( 10.*DBL_EPSILON, DBL_EPSILON*pow2(10., ceil(log10(max(fabs(x),max(fabs(y), DBL_EPSILON))+1.)) ) ) )) )
...
#define GET_F_DELTA2(x) ( (sizeof(x) == sizeof(float)) ? (max(F_DELTA, FLT_EPSILON * fabs(x))) : (max(D_DELTA, DBL_EPSILON * fabs(x))) )

#define isless2(x, y) (x < (y - GET_F_DELTA(x, y)))
#define ismore(x, y) (x > (y + GET_F_DELTA(x, y)))
#define iszero(x) (fabs(x) <= GET_F_DELTA2(x))
...
#define squared(x) pow2(x, 2.0) // added for convenience
#define pow2(x, y) (exp((y) * log(x))) //x^y == exponential(y * ln(x))
