...
typedef enum {SW_Adjust_Avg=1, SW_Adjust_StdErr}
  SW_AdjustMethods;
...
typedef struct {
     RealD wetdays[MAX_LAYERS],
           swc    [MAX_LAYERS],  /* soil water content cm/layer */
           swcm   [MAX_LAYERS],  /* soil water content cm/cm */
           swp    [MAX_LAYERS],  /* soil water potential */
           swa    [MAX_LAYERS], /* available soil water cm/layer, swc-(wilting point) */
           transp_total [MAX_LAYERS],
           transp_tree [MAX_LAYERS],
           transp_shrub [MAX_LAYERS],
           transp_grass [MAX_LAYERS],
           evap   [MAX_LAYERS],
           lyrdrain	[MAX_LAYERS],
           hydred_total	[MAX_LAYERS],
           hydred_tree	[MAX_LAYERS],  /* hydraulic redistribution cm/layer */
           hydred_shrub	[MAX_LAYERS],
           hydred_grass	[MAX_LAYERS],
           surfaceWater,
           total_evap, surfaceWater_evap, tree_evap, shrub_evap, grass_evap, litter_evap,
           total_int, tree_int, shrub_int, grass_int, litter_int,
           snowpack, snowdepth,
           et, aet, pet, deep,
...
typedef struct {
  ...
  /* current daily soil water related values */
    ...
    RealD swc           [TWO_DAYS] [MAX_LAYERS],
          snowpack      [TWO_DAYS], /* swe of snowpack, if accumulation flag set */
          snowdepth,
          transpiration_tree [MAX_LAYERS],
          transpiration_shrub [MAX_LAYERS],
          transpiration_grass [MAX_LAYERS],
          ...
          pet,
          aet,
          litter_evap, tree_evap, shrub_evap, grass_evap,
          litter_int, tree_int, shrub_int, grass_int,
          sTemp[MAX_LAYERS]; // soil temperature
